#This script is adapted from github.com/saitta-s/flow4D
# This script will read 4D Flow MRI in dicom format and output .vtk files
...
class DicomToVTK():
	...
	def Main(self):
		...
		print ("--- Reading Dicom Files and Meta Data ...")
		data, meta = ut.read_acquisition(self.Args.data_dir)
		arrayData = ut.seriesData_to_arrayData(data, meta)
		args = parser.parse_args()

		#-----------------------------------------------------------------------------------------------
		## Automatic detection of magnitude series
		print ("--- Automatically Detecing Magnitude and Phase Series ...")
		...
		#-------------------------------------------------------------------------------------------------------
		## Velocity adjustment
		print('--- Adjusting Velocity Units Depending on Vendors ...')
		if re.search('GE', meta['vendor'], re.IGNORECASE):
			velTemp /= self.Args.vel_factor  # m/s
		...
		for f in tqdm(range(meta['num_frames']), desc='Processing and saving frames'):
			...
			grid['Velocity'] = np.transpose(np.vstack((u.flatten(order='F'),v.flatten(order='F'),w.flatten(order='F'))))
		...
		if args.write_pcmra:
			...
			for i in range(meta['num_frames']):
				...
				pcmra += (args.mag_coeff * mag) ** 2 * ((args.phase_coeff * phase1) ** 2 + (args.phase_coeff * phase2) ** 2 + (args.phase_coeff * phase3) ** 2)
...
if __name__=="__main__":
        #Description
	parser = argparse.ArgumentParser(description="This script will generate a vtk fils from 4D Flow MRI Dicom Files")          
	...
	parser.add_argument('--vel_factor', type=float, default=1.0, help='scaling velocity by given factor.')
                        
        args=parser.parse_args()
        DicomToVTK(args).Main()
